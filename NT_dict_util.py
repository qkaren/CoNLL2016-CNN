...
if __name__ == "__main__":
        ...
        if relation["Type"] == "Explicit":
            ...
            if len(set(Arg1_sent_indices)) == 1 and len(set(Arg2_sent_indices)) == 1:  # 只考虑句子长度为1
                if set(Arg2_sent_indices) == set(Arg1_sent_indices):  # SS
                    ...
                    Arg1_count = 0
                    Arg2_count = 0
                    for constituent in get_constituents_with_label(syntax_tree, conn_head_indices, Arg1_token_indices,
                                                                   Arg2_token_indices):
    ...
    print("count: %d, No_Arg1_count: %d, No_Arg2_count: %d, lost_count: %d." % (
    count, No_Arg1_count, No_Arg2_count, lost_count))
    print(sorted(iter(No_Arg1_dict.items()), key=operator.itemgetter(1), reverse=True))
    print(sorted(iter(No_Arg2_dict.items()), key=operator.itemgetter(1), reverse=True))
    print(sorted(iter(cc_dict.items()), key=operator.itemgetter(1), reverse=True))
