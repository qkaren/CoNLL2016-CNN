...
from model_trainer.cnn_implicit_classifier.confusion_matrix import Alphabet
    ...
    class scorer(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            if acc_dev > scorer.dev_best_acc:
                ...
                model.save_weights('the_best_weights_333.h5' ,overwrite=True)
            if epoch % 3 == 0:
                print('the best dev is the_best_weights_333.h5:',nb_filter, filter_length1, filter_length2, filter_length3, 'epoch', scorer.dev_epoch_th, 'dev_best_acc:', scorer.dev_best_acc)
            print()
            sys.stdout.flush()
...
def test(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
    ...
    model.load_weights('the_best_weights_333.h5')
    dev_output = model.predict({'arg1': X_dev_1, 'arg2': X_dev_2, 'pos1': X_dev_pos_1, 'pos2': X_dev_pos_2},
                                       batch_size=batch_size)['output']
    acc_dev = (len([1 for i in range(len(y_dev)) if (TMP_equal_array(TMP_to_one(dev_output[i]),y_dev[i]))])) / len(y_dev)
    print()
...
if __name__ == '__main__':
    ...
    nb_filter = 1024
    # filter_list = [(4,4,4),(2,3,4),(2,7,12),(2,8,15)]
    # filter_list = [(1,2,3),(3,5,9),(3,4,5),(4,6,8)]
    filter_list = [(3,3,3),(3,3,3),(3,3,3),(3,3,3),(3,3,3),(3,3,3),(3,3,3)]
    for filter_length1, filter_length2, filter_length3 in filter_list:
        ...
        print('the final results is : ',nb_filter, filter_length1, filter_length2, filter_length3,dev_best_acc)
