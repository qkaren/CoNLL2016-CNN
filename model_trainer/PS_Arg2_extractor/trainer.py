...
from model_trainer.PS_Arg2_extractor.make_feature_file import ps_arg2_make_feature_file
from model_trainer.PS_Arg2_extractor.feature_functions import *
from pdtb_parse import PDTB_PARSE
from model_trainer.PS_Arg2_extractor import evaluation
...
class Trainer:
    ...
    def make_feature_file(self, train_pdtb_parse, dev_pdtb_parse):
        ...
        ps_arg2_make_feature_file(train_pdtb_parse, self.feature_function_list, self.train_feature_path)
        print("make %s feature file ..." % ("dev"))
        ps_arg2_make_feature_file(dev_pdtb_parse, self.feature_function_list, self.dev_feature_path)
...
if __name__ == "__main__":
    ...
    train_feature_path = config.PS_ARG2_TRAIN_FEATURE_OUTPUT_PATH
    dev_feature_path = config.PS_ARG2_DEV_FEATURE_OUTPUT_PATH

    ''' classifier '''
    classifier = Mallet_classifier(MaxEnt())

    ''' model path '''
    model_path = config.PS_ARG2_CLASSIFIER_MODEL

    ''' dev_result_file_path'''
    dev_result_file_path = config.PS_ARG2_DEV_OUTPUT_PATH
