...
def get_MPQA_polarity_score_vec(relation, Arg, parse_dict):
    ...
    # 字典
    polarity_dict = dict(list(zip(polarity_cate, list(range(5)))))
...
def _get_Arg_tense(relation, Arg, parse_dict):
    ...
    return _get_coarse_tense(get_tense_in_sent(list(zip(word_list, pos_list))))
...
def get_verb_pair(relation, parse_dict):
    arg1_verb_classes = _get_Arg_verb_classes(relation, "Arg1", parse_dict)
    arg2_verb_classes = _get_Arg_verb_classes(relation, "Arg2", parse_dict)

    if arg1_verb_classes == []:
        arg1_verb_classes = ["NULL"]
    if arg2_verb_classes == []:
        arg2_verb_classes = ["NULL"]

    verb_pair = []
    for v1_c in arg1_verb_classes:
        for v2_c in arg2_verb_classes:
            verb_pair.append("%s|%s" % (v1_c, v2_c))

    return verb_pair



def _get_Arg_verb_classes(relation, Arg, parse_dict):
    word_list = get_Arg_Words_List(relation, Arg, parse_dict)
    pos_list = get_Arg_POS_List(relation, Arg, parse_dict)
    ...
    for word, pos in zip(word_list, pos_list):
        if pos in verb_pos:
            verb_classes += get_verb_classes(word, pos)

    return verb_classes

# from nltk.corpus.reader import verbnet
# import os, config
# vn = verbnet.VerbnetCorpusReader(config.VERB_NET_PATH,os.listdir(config.VERB_NET_PATH))
# ['become-109.1', u'calibratable_cos-45.6-1', 'convert-26.6.2', 'escape-51.1', u'long-32.2-1']
def get_verb_classes(verb, pos):
    verb = util.lemma_word(verb, pos)
    return vn._lemma_to_class[verb][:1]

#  前面一个关系是不是显性的，如果是，用他的连接词，及 sense
# 连接词，没有则NULL
#dict[(DocID, sent1_index, sent2_index)] = [(conn_indices_string, conn, sense)]
def get_prev_context_conn(relation, parse_dict, implicit_context_dict):
    DocID = relation["DocID"]
