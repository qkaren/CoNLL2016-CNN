...
def pretainning():
    ...
    def data_process(relations_file,parses_file):
        ...
        for r in relations:
            ...
            for t in r['Arg2']['TokenList']:
                arg2_sent.add(t[3])
            if len(arg1_sent)>1 or len(arg2_sent)>1:
                continue
            for i in arg1_sent:
    ...
    def WE_process():
        ...
        f = open("nop_all_vocab_dict.txt",'w')
        f.write(dstr)
        f.close()
        return WE, pos_WE
    ...
    def embedding_process(train_relations,train_parses):
        ...
        y_s = np_utils.to_categorical(np.array([x[4] for x in data]), nb_classes=20)
        y_t = np_utils.to_categorical(np.array([x[5] for x in data]), nb_classes=20)
...
def fit_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
    ...
    class scorer(Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            sys.stdout.flush()
            print()
            print('Dev accuracy is:', round(acc_dev, 4), " at ", epoch, " the best before is ", scorer.dev_best_acc)
            if acc_dev > scorer.dev_best_acc:
                scorer.dev_best_acc = np.round(acc_dev, 4)
                scorer.dev_epoch_th = epoch + 1
                print("saving the best model-----the best dev is: ",nb_filter, filter_length1, filter_length2, filter_length3,scorer.dev_best_acc)
                model.save_weights('cnn_non_exp_sense_weights.h5' ,overwrite=True)
            if epoch % 3 == 0:
                print('the best dev is:', nb_filter, filter_length1, filter_length2, filter_length3,'epoch', scorer.dev_epoch_th, 'dev_best_acc:', scorer.dev_best_acc)
            print()
            sys.stdout.flush()
        def on_train_end(self, logs={}):
            print('the final best dev is:',nb_filter, filter_length1, filter_length2, filter_length3, 'dev_epoch', scorer.dev_epoch_th, 'dev_best_acc:', scorer.dev_best_acc)
    scorer = scorer()
...
if __name__ == '__main__':
    ...
    final = []
    nb_filter = 1024
    # filter_list = [(2,3,3),(2,5,10),(4,5,6)]
    # filter_list = [(4,8,12),(2,6,12),(3,9,15)]
    filter_list = [(4,8,12),(4,8,12),(4,8,12),(4,8,12),(4,8,12),(4,8,12)]
    for filter_length1, filter_length2, filter_length3 in filter_list:
        for batch_size in (64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64,64):
            '''  train '''
            print("posdim=100,maxlen=120")
            nnnn = int(sys.argv[1])
            with tf.device('/gpu:%d' % nnnn):
            ...
            print(batch_size,nb_filter,filter_length1, filter_length2, filter_length3,dev_best_acc)
            final.append((batch_size,filter_length1, filter_length2, filter_length3,dev_best_acc))
            print(final)
            print("posdim=100,maxlen=120")
        if dev_best_acc > 0.40:
            break
