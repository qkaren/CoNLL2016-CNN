...
import util
import config
from . import implicit_arg1_dict_util as dict_util
...
class Dict_creator:
    ...
    def create_term_dict(self, threshold = 1):
        ...
        # util.removeItemsInDict(dict, threshold)
        #字典keys写入文件
        util.write_dict_keys_to_file(dict_curr_first, config.IMPLICIT_ARG1_DICT_CURR_FIRST)
        util.write_dict_keys_to_file(dict_curr_last, config.IMPLICIT_ARG1_DICT_CURR_LAST)
        util.write_dict_keys_to_file(dict_prev_last, config.IMPLICIT_ARG1_DICT_PREV_LAST)
        util.write_dict_keys_to_file(dict_next_first, config.IMPLICIT_ARG1_DICT_NEXT_FIRST)
        util.write_dict_keys_to_file(dict_prev_last_curr_first, config.IMPLICIT_ARG1_DICT_PREV_LAST_CURR_FIRST)
        util.write_dict_keys_to_file(dict_curr_last_next_first, config.IMPLICIT_ARG1_DICT_CURR_LAST_NEXT_FIRST)
...
if __name__ == "__main__":
    ...
    Dict_creator(pdtb_parse).create_dict(dict_util.get_curr_lowercased_verbs, config.IMPLICIT_ARG1_DICT_LOWERCASE_VERBS)
    #
    Dict_creator(pdtb_parse).create_dict(dict_util.get_curr_lemma_verbs, config.IMPLICIT_ARG1_DICT_LEMMA_VERBS)

    Dict_creator(pdtb_parse).create_term_dict()
    #
    Dict_creator(pdtb_parse).create_dict(dict_util.get_curr_production_rule, config.IMPLICIT_ARG1_DICT_CURR_PRODUCTION_RULE)

    Dict_creator(pdtb_parse).create_dict(dict_util.get_prev_curr_production_rule, config.IMPLICIT_ARG1_DICT_PREV_CURR_PRODUCTION_RULE)

    Dict_creator(pdtb_parse).create_dict(dict_util.get_prev_curr_CP_production_rule, config.IMPLICIT_ARG1_DICT_PREV_CURR_CP_PRODUCTION_RULE)

    Dict_creator(pdtb_parse).create_dict(dict_util.get_curr_next_CP_production_rule, config.IMPLICIT_ARG1_DICT_CURR_NEXT_CP_PRODUCTION_RULE)

    Dict_creator(pdtb_parse).create_dict(dict_util.get_2prev_pos_lemma_verb, config.IMPLICIT_ARG1_DICT_2PREV_POS_LEMMA_VERB)

    Dict_creator(pdtb_parse).create_dict(dict_util.get_curr_first_to_prev_last_path, config.IMPLICIT_ARG1_DICT_CURR_FIRST_TO_PREV_LAST_PATH)
