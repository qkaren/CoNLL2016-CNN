    ...
    def data_process(relations_file,parses_file):
        ...
        for r in relations:
            ...
            if type == 'Explicit':
                ...
                connective_mapping = connective_raw
                try:
...
def build_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
    ...
    # model.add_node(Dense(1024),name='dense',inputs=['f1','connf'],merge_mode='concat')
    # model.add_node(Activation(activation),name='activation',input='dense')

    model.add_node(MaxPooling1D(pool_length=pool_length), name='mpooling_arg1',inputs=['cnn1_arg1','cnn2_arg1','cnn3_arg1'])
    model.add_node(MaxPooling1D(pool_length=pool_length), name='mpooling_arg2',inputs=['cnn1_arg2','cnn2_arg2','cnn3_arg2'])
    # model.add_node(Flatten(), name='marg1', input='mpooling_arg1')
    # model.add_node(Flatten(), name='marg2', input='mpooling_arg2')
    # model.add_node(Flatten(), name='connf', input='emb_conn')

    model.add_node(SimpleRNN(512),name='rnn',inputs=['mpooling_arg1','mpooling_arg2','emb_conn'])

    model.add_node(Dense(19),name='final', input='rnn')
...
def fit_model(batch_size,lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
    ...
    model = build_model(lr, activation, nb_filter, filter_length1, filter_length2, filter_length3,
              ...
              ndims = 300,pos_ndims = 100,conn_ndims=100,dist_ndims=32,ifconn_ndims=8,
...
if __name__ == '__main__':
    ...
    lr = 0.001
    final = []
    nb_filter_list = [512,1024]
    filter_list = [(2,2,2),(4,5,6),(3,7,10),(2,5,10),(3,10,15),(2,4,8)]
