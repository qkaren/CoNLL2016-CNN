...
class DiscourseParser():
    ...
    def parse(self):
        ...
        CNN_input_file_path = "cnn_nop.txt"
        ...
        cnn_noexp_output = cnn_test.test_nop(instances)
        ...
        self.non_explicit_relations = parser_util.cnn_non_explicit_read_model_output(cnn_noexp_output, self.non_explicit_relations)
        ...
        # adjacent_imp_list: [(DocID,sent1_index,sent2_index) ]
        # adjacent_imp_list =

        CNN_input_file_path = "cnn_imp.txt"
        instances = parser_util.format_for_impCNN(self.parse_dict, self.non_explicit_relations, CNN_input_file_path)
        with open("imp-cnn-inst.json", "w") as fout:
            fout.write(json.dumps(instances))
        # instances = parser_util.get_CNN_input(CNN_input_file_path)
        cnn_imp_output = cnn_test.test_imp(instances)
        ...
        # parser_util.put_feature_to_model(non_explicit_feat_path, non_explicit_model_path, non_explicit_model_output)
        # read model output, add sense for each non_explicit_relations
        self.non_explicit_relations = parser_util.cnn_non_explicit_read_model_output(cnn_imp_output,self.non_explicit_relations)
...
#    input_dataset = config.CWD + '../data/conll16st-en-zh-dev-train_LDC2016E50/conll16st-en-01-12-16-dev'
#    input_run = ''
