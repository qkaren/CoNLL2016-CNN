...
def get_all_connectives(documents):
    ...
    conns_list = sorted(conns_list, key=lambda x: x[0])
...
def constituent_print_feature(parse_dict, connectives, feature_function, to_file):
    ...
    for curr_index, connective in enumerate(connectives):
        ...
        if constituents == []:
            feature = Feature("", 0, {})
            example = Example("", feature)
            example.comment = "%s|%s" % (curr_index, " ")
            example_list.append(example)
...
def constituent_read_model_output(
    ...
    for relation_ID in list(relation_dict.keys()):
        list_ = relation_dict[relation_ID]
        Arg1_list = []
        Arg2_list = []
        for span, label in list_:
            if label == "Arg1":
...
def cnn_non_explicit_read_model_output(cnn_output,non_explicit_relations):
    import numpy as np
    pred_list = [cnn_config.Label_To_Sense[np.argmax(item)] for item in cnn_output]
    temp = []
    for sense, relation in zip(pred_list, non_explicit_relations):
        relation['Sense'] = [sense]
        temp.append(relation)
...
def attri_print_feature(parse_dict, relations, attribution_feat_func, to_file):
    ...
    for curr_index, relation in enumerate(relations):
        sys.stdout.flush()
        print("Extract Attribution Feature: %.2f%%.\r" % ((curr_index + 1)/total*100), end=' ')
...
def attri_read_model_output(attribution_feat_path, attribution_model_output, parse_dict, non_explicit_relations):
    ...
    for (relation_ID, Arg) in list(relation_dict.keys()):
...
def ps_arg2_extractor_read_model_output(PS_Arg2_feat_path, PS_Arg2_model_output, parse_dict, PS_conns_list_args):
    ...
    # remove the attribution part for each argument of the relation
    for (relation_ID, Arg) in list(relation_dict.keys()):
        list_ = relation_dict[(relation_ID, Arg)]#[([1, 2],'yes')....]
        ...
        if arg_list == []:
            if Arg == "Arg1":
                relation_dict[(relation_ID, Arg)] = list_[-1][0]
            else:
                relation_dict[(relation_ID, Arg)] = list_[0][0]
        else:
            relation_dict[(relation_ID, Arg)] = arg_list
...
def format_for_CNN(parse_dict, adjacent_non_exp_list, to_file):
    ...
    with open(to_file, "w") as fout:
        ...
        for DocID, sent1_index, sent2_index in adjacent_non_exp_list:
            ...
            Arg1_word_list, Arg1_pos_list = _get_NonExplicit_original_Arg(parse_dict, DocID, sent1_index)
            Arg2_word_list, Arg2_pos_list = _get_NonExplicit_original_Arg(parse_dict, DocID, sent2_index)

            sense = "NULL"

            line = "%s|~|%s|~|%s|~|%s|~|%s\n" % \
                   (" ".join(Arg1_word_list), " ".join(Arg2_word_list), " ".join(Arg1_pos_list), " ".join(Arg2_pos_list), sense)

            fout.write(line)
            ins_ret.append((Arg1_word_list, Arg2_word_list, Arg1_pos_list, Arg2_pos_list, sense))
    return ins_ret

def format_for_impCNN(parse_dict, non_explicit_relations, to_file):
    ...
    with open(to_file, "w") as fout:
        ...
        for relation in non_explicit_relations:
            DocID = relation["DocID"]
            sent1_list = relation['Arg1']['TokenList']
            sent2_list = relation['Arg2']['TokenList']
            Arg1_word_list, Arg1_pos_list = get_imp_arg_rawtext(parse_dict, DocID, sent1_list)
            Arg2_word_list, Arg2_pos_list = get_imp_arg_rawtext(parse_dict, DocID, sent2_list)
...
def get_imp_arg_rawtext(parse_dict,DocID,arg_list):
    sent_word_offset = []
    for arg in arg_list:
        sent_word_offset.append((arg[3],arg[4]))
    word_list = [parse_dict[DocID]["sentences"][sent]["words"][word][0] for sent, word in sent_word_offset]
    pos_List = [parse_dict[DocID]["sentences"][sent]["words"][word][1]["PartOfSpeech"] for sent, word in sent_word_offset]
    return (word_list,pos_List)

def _get_NonExplicit_original_Arg(parse_dict, DocID, sent_index):
    index_word_pair = [(index, word[0]) for index, word
                       in enumerate(parse_dict[DocID]["sentences"][sent_index]["words"])]
    #去两边的标点
    # index_word_pair = util.list_strip_punctuation(index_word_pair)
    #
    word_list = [word for index, word in index_word_pair]
    #
    pos_List = [parse_dict[DocID]["sentences"][sent_index]["words"][index][1]["PartOfSpeech"] for index, word in index_word_pair]

    return word_list, pos_List



def get_CNN_input(in_file):
    instances = []
    with open(in_file) as fin:
        for line in fin:
            ...
            if line == "":
                continue
            Arg1_word, Arg2_word, Arg1_pos, Arg2_pos, sense = line.split("|~|")
            instances.append((Arg1_word, Arg2_word, Arg1_pos, Arg2_pos, sense))
